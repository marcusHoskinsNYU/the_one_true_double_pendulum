...
savepath='/Users/Krishna/Documents/repositories/the_one_true_double_pendulum/figures/rk4_convergence'
...
for i in range(len(timesteps)):
    ...
    file=np.loadtxt(f'{path}/results/rk4_part2/partb_rk4_80s_{timesteps[i]}us_timesteps.csv', delimiter=',', skiprows=1)
    files.append(file)
...
def plot_err(timesteps=timesteps, files=files, q=0, tfinal=10):
    '''
...
s
    ...
    plt.legend()
    plt.savefig(f'{savepath}/rk4_integrator_{x[q]}Err_{tfinal}s_timesteps.png')
...
def plot_rms_err(timesteps=timesteps, files=files, q=0, tfinal=12,log=False ):
    '''
    ...
    if log==True:
        ...
        plt.title(f'log log plot of root mean squared error after {tfinal}s')
        plt.savefig(f'{savepath}/rk4_{x[q]}rms_err_vs_stepsize_after_{tfinal}s.png')
        ...
        plt.savefig(f'{savepath}/rk4_{x[q]}_log_rms_err_vs_stepsize_after_{tfinal}s.png')

q=0

#plot_err(timesteps=timesteps, files=files, q=q, tfinal=15)

plot_rms_err(timesteps=timesteps, files=files, q=q, tfinal=9,log=True )
